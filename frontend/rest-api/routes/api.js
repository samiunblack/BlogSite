...
module.exports = {
    getAllBlogPosts: function(callback) {
        ...
        BlogPostModel.find({dateTimestamp: {$lte: now}}, "title urlTitle dateTimestamp tags")
    ...
    getBlogPostsByTag: function(tag, callback) {
        ...
        BlogPostModel.find({tags: tag.toLowerCase(), dateTimestamp: {$lte: now}}, "title urlTitle dateTimestamp tags")
        .sort({dateTimestamp: -1})
        .exec(function(error, posts) {
            if (error) {
            callback({getDataError: true})
